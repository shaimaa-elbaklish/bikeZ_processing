...
# def in_bike_lane(self, s, d, tolerance=0.4):
#     bl = self.bike_lane
#     if bl is None:
#         return False   # no bike lane on this segment
#     s_min, s_max = bl['s_domain']
#     if not (s_min <= s <= s_max):
#         return np.nan  # outside boundary coverage

#     d_bnd    = float(bl['d_boundary_spline'](s))
#     side     = bl['side']
#     w_bike   = bl['w_bike']

#     # Bike lane occupies [d_bnd, d_bnd + side*w_bike]
#     d_near   = d_bnd
#     d_far    = d_bnd + side * w_bike
#     d_lo     = min(d_near, d_far) - tolerance
#     d_hi     = max(d_near, d_far) + tolerance

#     return bool(d_lo <= d <= d_hi)
